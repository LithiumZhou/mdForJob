...
| 对比项       | 快速转发（Fast）                           | 标准转发（Standard）                                   |
| ------------ | ------------------------------------------ | ------------------------------------------------------ |
| **入口方法** | `-forwardingTargetForSelector:`            | `-methodSignatureForSelector:` + `-forwardInvocation:` |
| **主要用途** | 快速将消息交给“另一个对象”处理（类似代理） | 自定义消息转发逻辑，如修改参数、动态调用、日志记录等   |
| **性能**     | 快（几乎和正常调用差不多）                 | 慢（需要构建 `NSInvocation` 对象）                     |
| **灵活性**   | 只能更换目标对象                           | 可以修改选择器、参数，甚至实现调用前后逻辑             |
| **系统行为** | 不会再调用标准转发                         | 如果返回 `nil`，才会进入标准转发                       |
| **典型用途** | 消息委托、轻量代理                         | 消息重定向、AOP、Mock、消息记录等                      |

## ***oc和swift有什么区别

**🧩 二、从 5 个维度对比（主干逻辑）**

| 维度            | Objective-C                                         | Swift                                    | 总结                                |
| --------------- | --------------------------------------------------- | ---------------------------------------- | ----------------------------------- |
| **语言类型**    | 动态语言（runtime 决定一切）                        | 静态语言（编译期决定类型）               | OC 灵活，Swift 安全                 |
| **语法特性**    | 早期 C 语法 + Smalltalk 消息机制                    | 现代语法 + 泛型 + 函数式                 | Swift 更简洁现代                    |
| **内存管理**    | ARC 自动引用计数，但允许手动干预                    | ARC 更严格、自动管理强                   | Swift 更安全不易泄漏                |
| **运行时机制**  | 完全依赖 ObjC runtime（消息转发、Method Swizzling） | 编译期确定函数表，runtime 支持有限       | OC 可运行时魔改，Swift 可预测性更好 |
| **与 C 的关系** | 直接超集，可混编 C/C++                              | 通过 Bridging 与 OC 交互，不支持直接 C++ | Swift 更封闭但跨平台                |

## ***charles抓包原理

### 1. 代理设置

- **配置手机：** 用户需要在手机的 Wi-Fi 设置中手动配置 **HTTP 代理**，将代理服务器指向运行 Charles 的电脑的 **IP 地址**和 Charles 的 **端口号**（默认通常是 8888）。
- **流量重定向：** 一旦设置完成，手机上的所有网络请求（包括应用程序和浏览器产生的）都不会直接发送到目标服务器，而是先**全部发送到 Charles 运行的电脑上**。Charles 就像一个守卫，截获了所有出站和入站的流量。

### 2. HTTP 抓包原理

对于非加密的 **HTTP** 流量，原理相对简单：

- **截获请求：** 手机将 HTTP 请求发送给 Charles。
- **转发请求：** Charles 接收到请求后，将其转发给目标服务器。
- **截获响应：** 服务器将 HTTP 响应返回给 Charles。
- **转发响应：** Charles 接收到响应后，将其转发给手机。
- **记录和展示：** 在这一过程中，Charles 记录了完整的请求（URL、头部、参数、请求体）和响应（状态码、头部、响应体），并在其界面上结构化地展示出来。

### 3. HTTPS 抓包（SSL/TLS）原理

对于加密的 **HTTPS** 流量，过程稍微复杂，需要进行“解密”操作，这就是 Charles 称为“中间人”的关键：

- **安装证书：** 用户需要在手机上安装并信任 Charles 的 **根证书（Root Certificate）**。这是进行 HTTPS 抓包的前提条件。
- **Charles 扮演服务器：** 当手机尝试连接一个 HTTPS 服务器时：
  - Charles 会**伪装成目标服务器**，向手机出示一个**由 Charles 自己的根证书签发的伪造证书**。
  - 因为用户已经在手机上信任了 Charles 的根证书，手机会认为这个伪造的连接是安全的，并使用 Charles 提供的公钥建立加密通信（即手机和 Charles 之间建立了一个 SSL/TLS 连接）。
- **Charles 扮演客户端：** Charles 同时会和真正的目标服务器建立另一个独立的 SSL/TLS 连接，它使用自己的私钥和目标服务器进行正常的加密通信。
- **解密与加密：**
  - Charles 收到手机发送的加密数据后，**使用 Charles 自己的私钥进行解密**，获取明文数据。
  - Charles 将明文数据用目标服务器的公钥**重新加密**，发送给目标服务器。
  - 反之，Charles 收到目标服务器的加密数据后，**使用目标服务器的公钥进行解密**，获取明文数据。
  - Charles 将明文数据用 Charles 自己的公钥**重新加密**，发送给手机。

通过这两个独立的 SSL/TLS 连接和解密/重新加密的过程，Charles 成功地充当了“中间人”，能够查看和修改原本加密的 HTTPS 流量的明文内容。
...
一个短有效期token（30min），一个长有效期token（7天）。当你每次登陆之后就会返两个token给前端，短token是一个jwt token,长token是一个并且存在redis里七天有效期的token，为什么这么设计我后面讲。

比如说你每次需要鉴权的时候都是把这个短token传给后端去校验一下有效期，如果发现这个token过期了，就会返一个特定的code给前端，前端捕捉到这个code就知道是短token过期了，这个时候就去访问一个接口，拿这个长token去换一个新的短token存在authorization字段。

我理解的双token的优势有两点

* 假如短token被恶意截取了，也只有短时间内有效，提高了安全性（主要是因为短token用的频繁，长token用的不多，被恶意截取的概率小一些）。
...
异步转码就是说，用户把文件传完了就行，然后用户就可以填封面和简介那种信息，最后直接点立即投稿就开始异步转码，用户点完了就完事。缺点就是可能投稿之后然后转码失败。

转码就是指把一个mp4文件给转码成多个ts文件和一个m3u8文件，m3u8里面存的是就是所有ts文件名和对应的时长，相当于一个索引文件。
