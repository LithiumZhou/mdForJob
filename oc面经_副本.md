...
5.  **自动取消下载**：如果用户在图片完成下载前就将其滚出屏幕，SDWebImage 会自动取消这次下载请求，以节省系统资源。它也完美地处理了 Cell 复用带来的图片错乱问题。
6.  **图片解码与转换**：它会在后台对图片进行解码，避免了图片显示前一瞬间可能出现的 UI 卡顿。它还允许你在图片下载后、显示和缓存前，对图片进行转换（如调整大小、裁剪、添加圆角或应用滤镜）。
7.  **广泛的格式支持**：除了标准的 PNG 和 JPEG，它内置了对 GIF 动图的支持，并且通过插件可以支持 WebP、HEIC、AVIF 甚至 SVG 等现代格式。
8.  **渐进式图片加载**：它可以像网页浏览器那样，在图片下载过程中就逐步显示出来，提升了用户的感知性能。
...
SDWebImage 远不止是一个图片下载器。它是一个完整的、经过实战检验的图片管理框架，它将多线程、缓存和性能优化的复杂性完全封装了起来。通过使用它，开发者可以节省数百小时的工作量，并为他们的应用程序提供明显更好、更流畅的用户体验。
...
1.  **浏览器缓存 (Browser Cache)**: 浏览器会先检查自己的缓存，看看最近有没有访问过这个域名。
2.  **操作系统缓存 (OS Cache)**: 如果浏览器缓存没有，操作系统会检查自己的缓存。
3.  **Hosts 文件**: 这是一个位于操作系统中的特殊文本文件，用户可以手动在里面定义域名和 IP 的对应关系。系统会检查这个文件。
4.  **路由器缓存 (Router Cache)**: 如果以上都没有，请求可能会被发送到你的路由器，路由器通常也有自己的 DNS 缓存。

**结果**：如果以上任何一个缓存命中，查询就直接结束，系统将使用找到的 IP 地址。如果全都没有命中，则进入下一步。

#### 第 2 步：向本地 DNS 服务器发起请求

本机将查询请求发送给在网络设置中配置好的**本地 DNS 服务器**。从这里开始，本地 DNS 服务器将接管所有复杂的查询工作。这个过程对你的电脑来说是**递归查询 (Recursive Query)**，意思是：你（本机）只问一次，本地 DNS 服务器必须给你一个最终答案（IP 地址或“找不到”的错误）。

#### 第 3 步：本地 DNS 服务器的迭代查询之旅

本地 DNS 服务器收到请求后，会开始一个**迭代查询 (Iterative Query)** 的过程，它会像侦探一样，一步步地询问，直到找到答案：
