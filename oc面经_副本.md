...
简单来说，**SDWebImage 是一个功能强大、特性丰富的异步图片下载和缓存库**。

想象一下，你正在开发一个像 Instagram 或新闻阅读器这样的应用，需要在可滚动的列表（`UITableView` 或 `UICollectionView`）中显示几十甚至上百张来自网络的图片。如果尝试手动实现这个功能，你会遇到一连串噩梦般的问题：

1.  **UI 卡顿**：如果你在主线程下载图片，你的 App 界面会在下载过程中完全冻结。
2.  **复杂的多线程管理**：为了避免卡顿，你需要管理后台线程去下载，下载完成后再切回主线程更新 UI，这个过程非常容易出错。
3.  **没有缓存**：每次图片出现在屏幕上时都重新下载一遍，这是极其低效的，会浪费用户的流量和电量。
4.  **Cell 复用问题**：在滚动列表中，Cell 会被复用。当你快速滚动时，一个 Cell 可能会错误地显示上一个下载请求的旧图片。
5.  **处理不同格式**：如果要支持 GIF 动图，或者像 WebP 这样的现代图片格式，就需要做额外的工作。

SDWebImage 用一套简洁、优雅的 API 解决了所有这些问题（以及更多）。
