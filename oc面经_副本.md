...
*   **调用时机**:
    *   在一个 ViewController 实例的整个生命周期中，**有且仅有一次**被调用。
    *   发生在它的 `view` 从无到有，被成功加载进内存之后。此时，所有通过 Storyboard/XIB 拖拽的 UI 控件 (`IBOutlet`) 都已经创建好并连接上了。

*   **核心用途**: **所有只需要做一次的、重量级的“准备”工作。**
    *   **UI 初始化 (非布局)**：
        *   设置控件的**初始内容**：`self.title = @"个人中心";`，`nameLabel.text = @"默认名称";`
        *   设置控件的**样式**：`myButton.backgroundColor = [UIColor blueColor];`
        *   创建并**添加子视图**：`[self.view addSubview:self.customView];`
        *   **设置 AutoLayout 约束**：这是定义约束规则的最佳位置，因为约束只需要被创建一次。
    *   **数据初始化**：
        *   **发起网络请求**：去加载这个页面所必需的核心数据。
        *   初始化本地数据源，比如创建一个空的 `NSMutableArray`。
    *   **其他一次性设置**：
        *   **添加手势识别器 (`UITapGestureRecognizer`)**。
        *   **注册 `NSNotificationCenter` 的观察者**。

*   **【禁忌】**:
    *   **不要在这里做任何依赖于视图最终 `frame` 或 `bounds` 的计算**。因为此时的尺寸可能还是 Storyboard 里的设计尺寸，或者是一个默认值，AutoLayout 引擎还没有完成最终的计算。
...
*   **调用时机**:
    *   在 `view` **即将**被添加到视图层级、显示在屏幕上之前。
    *   每次页面要显示时，都会被调用。比如，从下一个页面 `pop` 回来，或者从另一个 Tab 切换回来。

*   **核心用途**: **每次登台表演前的“最后检查”和“状态刷新”。**
    *   **数据刷新**：
        *   刷新 `TableView` 或 `CollectionView` 的数据 (`[self.tableView reloadData];`)，以确保显示的是最新的内容（比如用户在详情页修改了某个数据，返回列表页时需要同步）。
        *   根据最新的模型数据，更新 UI 控件的内容。
    *   **UI 状态更新**：
        *   根据当前 App 的状态（比如登录状态、主题模式），调整 UI 的显示。
        *   **设置导航栏的样式**（隐藏/显示、颜色、按钮），因为它可能会影响下一个页面，所以需要在“即将消失”时把它改回去。
    *   **启动/恢复任务**：
        *   **注册 KVO (键值观察)**。
        *   启动一些需要页面可见才能运行的动画或定时器。
        *   开始监听键盘通知。
...
*   **调用时机**:
    *   在 `view` **已经完全**显示在屏幕上，并且所有出现动画都执行完毕之后。
    *   与 `viewWillAppear` 成对出现，每次显示都会调用。

*   **核心用途**: **已经登上舞台，可以开始“正式表演”了，尤其是一些比较耗时或需要交互的表演。**
    *   **触发重量级操作**：
        *   发起一些**非核心**的、可以延迟加载的网络请求。
        *   启动一些复杂的、消耗资源的计算，避免在 `viewDidLoad` 或 `viewWillAppear` 中执行而阻塞界面出现。
    *   **需要 UI 完全可见的操作**：
        *   **弹出 Alert (警示框)** 或 ActionSheet。
        *   **Present (模态弹出)** 另一个 ViewController。
        *   播放视频或音频。
        *   让某个文本框成为第一响应者，**弹出键盘** (`[self.textField becomeFirstResponder];`)。
    *   **性能分析**：
        *   在这里记录一个时间戳，用于计算页面的**加载耗时**。
...
*   **调用时机**:
    *   在 `view` **即将**从视图层级中移除、从屏幕上消失之前。
    *   当用户 `push` 到下一个页面，或者 `present` 一个新页面，或者切换到另一个 Tab 时，都会被调用。

*   **核心用途**: **下台前的“清理”和“状态保存”，与 `viewWillAppear` 的操作正好相反。**
    *   **保存状态**：
        *   保存用户在文本框里输入但尚未提交的草稿。
    *   **UI 状态重置**：
        *   **恢复导航栏的样式**，把它改成一个通用的状态，以免“污染”即将出现的新页面。
    *   **暂停/撤销任务**：
        *   **移除 KVO (键值观察)**。
        *   **使定时器失效 (`[self.timer invalidate];`)**，这是一个非常重要的操作，可以防止内存泄漏和不必要的后台计算。
        *   停止监听键盘通知。
...
*   **调用时机**:
    *   在 `view` **已经完全**从屏幕上消失，并且所有消失动画都执行完毕之后。

*   **核心用途**: **已经回到后台，可以停止一些重量级的、只有在前台才有意义的服务。**
    *   **停止服务**：
        *   停止视频/音频的播放。
        *   停止 GPS 的持续定位。
        *   停止一些复杂的数据流监听。
